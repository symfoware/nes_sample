...
mainLoop:					; メインループ
	...
	lda #$00
	sta $4016
	sta KeyBuffer	; キーバッファ初期化
	ldx #$08
	jmp getPadBitSub

getPadBit:
	asl <KeyBuffer	; キーバッファ左シフト
getPadBitSub:
	; パッド入力チェックループ
	lda $4016  		; ボタン
	and #1     		; AND #1
	...
	sta <KeyBuffer
	dex
	bne getPadBit	; 8回繰り返す(A,B,Select,Start,上,下,左,右)

	lda <KeyBuffer
	lsr <KeyBuffer	; キーバッファ右シフト
	and #1     		; AND #1
	beq lsr_A1		; 0ならばlsr_A1へジャンプ
	jsr RIGHTKEYdown
lsr_A1:
	lda <KeyBuffer
	lsr <KeyBuffer	; キーバッファ右シフト
	and #1     		; AND #1
	beq lsr_A2		; 0ならばlsr_A2へジャンプ
	jsr LEFTKEYdown
lsr_A2:
	lda <KeyBuffer
	lsr <KeyBuffer	; キーバッファ右シフト
	and #1     		; AND #1
	beq lsr_A3		; 0ならばlsr_A3へジャンプ
	jsr DOWNKEYdown
lsr_A3:
	lda <KeyBuffer
	lsr <KeyBuffer	; キーバッファ右シフト
	lsr <KeyBuffer	; キーバッファ右シフト(Startをスキップ)
	lsr <KeyBuffer	; キーバッファ右シフト(Selectをスキップ)
	and #1     		; AND #1
	beq lsr_A4		; 0ならばlsr_A4へジャンプ
	jsr UPKEYdown
lsr_A4:
	lda <KeyBuffer
	lsr <KeyBuffer	; キーバッファ右シフト
	and #1     		; AND #1
	beq lsr_A5		; 0ならばlsr_A5へジャンプ
	jsr BKEYSound
lsr_A5:
	lda <KeyBuffer
